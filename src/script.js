...
const repo = {
  ...
  deleteTask: async (uid, id) => {
    if (DB_TYPE === "realtime")
      return dbRT.ref(`users/${uid}/tasks/${id}`).remove();
    return dbFS
      .collection("users")
      .doc(uid)
      .collection("tasks")
...
function processList(type, list, container) {
  ...
  if (s.filterType === "name" && s.filterValue) {
    processedList = processedList.filter((t) =>
      t.name.toLowerCase().includes(s.filterValue.toLowerCase())
    );
  ...
  if (type === "pending")
    els.pendingHeader.textContent = `Pendientes (${processedList.length})`;
  else
    els.completedHeader.textContent = `Completadas (${processedList.length})`;
  ...
  container.innerHTML =
    paginated
      .map((t) => {
        ...
        // Formato para fecha de finalización
        const completedDate = t.completedAt ? new Date(t.completedAt).toLocaleString([], { day: '2-digit', month: '2-digit', year: 'numeric', hour: '2-digit', minute: '2-digit' }) : "--";
    ...
    <div class="card-body p-2 d-flex align-items-center">
        ...
        <div class="flex-grow-1" onclick="openEdit('${t.id}')" style="cursor:pointer; min-width: 0;">
            <div class="mb-0 text-truncate ${t.isCompleted ? "text-decoration-line-through text-muted" : "fw-bold text-dark"}" style="font-size: 0.9rem;">
                ${t.name}
            </div>
            <div class="d-flex flex-wrap gap-x-2 text-muted single-line-meta" style="font-size: 0.68rem; white-space: nowrap;">
                <span class="d-flex align-items-center">
                    <i class="bi bi-plus-circle me-1"></i><b>Cda:</b>&nbsp;${createdDate}
                </span>
                <span class="d-flex align-items-center ${!t.isCompleted && t.dateTime < Date.now() ? "text-danger fw-bold" : ""}">
                    <i class="bi bi-alarm me-1"></i><b>Vto:</b>&nbsp;${dueDate}
                </span>
                ${t.isCompleted ? `
                <span class="d-flex align-items-center text-success fw-semibold">
                    <i class="bi bi-check-all fs-6 me-1"></i><b>Fda:</b>&nbsp;${completedDate}
                </span>` : ""}
            </div>
        </div>
        ...
        <div class="d-flex gap-0">
            ${!t.isCompleted 
                ? `<button class="btn btn-action-task p-1" onclick="quickAction('${t.id}', 'complete')"><i class="bi bi-check2-circle fs-5 text-success"></i></button>`
                : `<button class="btn btn-action-task p-1" onclick="quickAction('${t.id}', 'uncomplete')"><i class="bi bi-arrow-counterclockwise fs-5 text-warning"></i></button>`
            }
      ...
      .join("") ||
    '<div class="text-center p-4 text-muted">No hay tareas</div>';
...
window.toggleSelectAll = (listType, isChecked) => {
  ...
  const listTasks = tasks.filter((t) =>
    listType === "pending" ? !t.isCompleted : t.isCompleted
  );
...
function updateBulkBarUI() {
  ...
  if (hasSelection) {
    ...
    document.getElementById(
      "selected-count"
    ).textContent = `${count} seleccionadas (${
      bulkSelection.type === "pending" ? "Pendientes" : "Completadas"
    })`;
...
window.processBulkAction = async (action) => {
  ...
  bulkSelection.type = null;
  document
    .querySelectorAll(".bulk-checkbox-main")
...
window.openEdit = (id) => {
  ...
  document.getElementById("saveTaskBtn").style.display = isDone
    ? "none"
    : "block";
  document.getElementById("modalTitle").textContent = isDone
  ...
  if (!isDone) {
    ...
    document.getElementById("taskDateTime").min =
      t.dateTime < Date.now() ? taskDateLocal : nowLocal;
  }
...
function updateAlerts() {
  ...
  const threshold =
    Date.now() + userSettings.bellValue * mult[userSettings.bellUnit];
  ...
  els.notifList.innerHTML =
        ...
        <li class="dropdown-item border-bottom p-3">
            <div class="d-flex justify-content-between align-items-center">
                <div onclick="openEdit('${
                  t.id
                }')" style="cursor:pointer" class="flex-grow-1">
                    <span class="text-danger fw-bold small">⚠️ ${
                      t.name
                    }</span><br>
                    <small class="text-muted">${t.dt.toLocaleString()}</small>
                </div>
                <div class="d-flex gap-1 ms-2">
                    <button class="btn btn-sm btn-success py-0 px-2" onclick="quickAction('${
                      t.id
                    }', 'complete')" title="Completar">
                        <i class="bi bi-check"></i>
                    </button>
                    <button class="btn btn-sm btn-danger py-0 px-2" onclick="deleteTask('${
                      t.id
                    }')" title="Eliminar">
    ...
    `
      ...
      .join("") ||
    '<li class="dropdown-item text-muted text-center py-2">Sin alertas</li>';
}
...
document.getElementById("bellEnabled").onchange = (e) =>
  (document.getElementById("bellConfigContainer").style.display = e.target
    .checked
    ? "block"
    : "none");
document.getElementById("emailSummaryEnabled").onchange = (e) =>
  (document.getElementById("emailSummaryConfigContainer").style.display = e
    .target.checked
    ? "block"
    : "none");
...
document.getElementById("selectPendientes").onchange = (e) =>
  handleFilterChange("pending", e.target.value);
document.getElementById("selectCompletadas").onchange = (e) =>
  handleFilterChange("completed", e.target.value);
const getPriorityColor = (p) =>
  ({ urgente: "danger", alta: "warning", media: "info", baja: "secondary" }[p]);
window.deleteTask = (id) =>
  confirm("¿Eliminar?") && repo.deleteTask(firebaseUser.uid, id);

els.loginBtn.onclick = () =>
  auth.signInWithPopup(new firebase.auth.GoogleAuthProvider());
els.logoutBtn.onclick = () => auth.signOut();
document
  .querySelectorAll('[data-bs-target="#settingsModal"]')
  .forEach(
    (btn) =>
